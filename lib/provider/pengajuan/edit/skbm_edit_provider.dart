...
class SkbmEditProvider extends ChangeNotifier {
  ...
  Future<Map<String, dynamic>?> fetchSkbmById(String detailId) async {
    try {
      ...
      final uri = Uri.parse('$baseUrl/skbm/$detailId');
  ...
  void fillFormWithExistingData({
    ...
    try {
      ...
      selectedHubunganId = _findIdByName(
        hubunganList,
      ...
      selectedKelaminId = _findIdByName(
        jkList,
        (skuData['jk'] ?? skuData['jenis_kelamin'] ?? '').toString(),
        (item) => item.jenisKelamin,
        (item) => item.id,
      );

      selectedPekerjaanId = _findIdByName(
        pekerjaanList,
      ...
      selectedStatusId = _findIdByName(
        statusList,
      ...
      // Handle error in filling form
    }
  }

  int? _findIdByName<T>(List<T> list, String? targetName,
