...
class SkpProvider extends ChangeNotifier {
  // create
  ...
  int? selectedPekerjaanTerdahuluIdCreate;
  int? selectedPekerjaanSekarangIdCreate;
  ...
  int? selectedPekerjaanTerdahuluIdUpdate;
  int? selectedPekerjaanSekarangIdUpdate;
  ...
  int? selectedPekerjaanTerdahuluId(bool isUpdate) => isUpdate
      ? selectedPekerjaanTerdahuluIdUpdate
      : selectedPekerjaanTerdahuluIdCreate;
  int? selectedPekerjaanSekarangId(bool isUpdate) => isUpdate
      ? selectedPekerjaanSekarangIdUpdate
      : selectedPekerjaanSekarangIdCreate;
  ...
  void setSelectedPekerjaanTerdahuluIdCreate(int? value) {
    selectedPekerjaanTerdahuluIdCreate = value;
    notifyListeners();
  }

  void setSelectedPekerjaanSekarangIdCreate(int? value) {
    selectedPekerjaanSekarangIdCreate = value;
    notifyListeners();
  }
  ...
  void setSelectedPekerjaanTerdahuluIdUpdate(int? value) {
    selectedPekerjaanTerdahuluIdUpdate = value;
    notifyListeners();
  }

  void setSelectedPekerjaanSekarangIdUpdate(int? value) {
    selectedPekerjaanSekarangIdUpdate = value;
    notifyListeners();
  }
  ...
  void setSelectedNikCreate(
    ...
    selectedPekerjaanTerdahuluIdCreate = anggota.pekerjaan;
    selectedStatusIdCreate = anggota.status;
    notifyListeners();
  }

  void setSelectedNikUpdate(
    int index,
    int id,
    NikHive anggota, {
    required List<Datum> jkList,
    ...
    selectedPekerjaanTerdahuluIdUpdate = anggota.pekerjaan;
    selectedPekerjaanSekarangIdUpdate = anggota.pekerjaan;
  ...
  Future<int> submitFormCreate(BuildContext context) async {
    ...
    try {
      if (selectedHubunganIdCreate == null ||
          ...
          selectedPekerjaanTerdahuluIdCreate == null ||
      ...
      final Map<String, dynamic> data = {
        ...
        'pekerjaan_terdahulu': selectedPekerjaanTerdahuluIdCreate,
        'pekerjaan_sekarang': selectedPekerjaanSekarangIdCreate,
        'status_perkawinan': selectedStatusIdCreate,
        'alamat': alamatControllerCreate.text,
      };

      final uri = Uri.parse('$baseUrl/skp');
  ...
  Future<Map<String, dynamic>?> fetchSkpById(String detailId) async {
    try {
      final token = await _getToken();
      if (token == null || token.isEmpty) {
        throw Exception('Token tidak ditemukan. Silakan login ulang.');
      ...
      final uri = Uri.parse('$baseUrl/skp/$detailId');
...
// Method untuk mengisi form UPDATE dengan data yang sudah ada
  ...
  void fillFormWithExistingDataUpdate({
    ...
    try {
      ...
      selectedPekerjaanTerdahuluIdUpdate = findIdByName(
        pekerjaanList,
        skuData['pekerjaan_terdahulu']?.toString(),
        (item) => item.namaPekerjaan,
        (item) => item.id,
      );
      debugPrint(
          'selectedPekerjaanTerdahuluIdUpdate: $selectedPekerjaanTerdahuluIdUpdate');

      selectedPekerjaanSekarangIdUpdate = findIdByName(
        pekerjaanList,
        skuData['pekerjaan_sekarang']?.toString(),
        (item) => item.namaPekerjaan,
        (item) => item.id,
      );
      debugPrint(
          'selectedPekerjaanSekarangIdUpdate: $selectedPekerjaanSekarangIdUpdate');
...
// Cek apakah ada yang null
  ...
  Future<int> updateForm(BuildContext context, String detailId) async {
    ...
    try {
      ...
      if (selectedPekerjaanTerdahuluIdUpdate == null)
      ...
      final Map<String, String> data = {
        ...
        'pekerjaan_terdahulu':
            selectedPekerjaanTerdahuluIdUpdate?.toString() ?? '',
        'pekerjaan_sekarang':
            selectedPekerjaanSekarangIdUpdate?.toString() ?? '',
  ...
  void resetFormCreate() {
    ...
    selectedKelaminIdCreate = null;
    selectedPekerjaanTerdahuluIdCreate = null;
    selectedPekerjaanSekarangIdCreate = null;
    selectedStatusIdCreate = null;
  ...
  void resetFormUpdate() {
    ...
    selectedPekerjaanTerdahuluIdUpdate = null;
    selectedPekerjaanSekarangIdUpdate = null;
